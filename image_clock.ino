...
#define DISP_W        240
#define DISP_H        320
#define MAX_FNAME     64        // max chars per path (increased for safety)
...
// ── Slideshow state ───────────────────────────────────────────
bool          sdOK        = false;
char**        imageList   = nullptr;   // ← dynamic, no fixed ceiling
int           imageCount  = 0;
int           curImg      = 0;
...
.thumb-wrap{position:relative;width:60px;height:80px;background:#1e1e38;border-radius:4px;overflow:hidden}
.thumb-wrap canvas{width:60px;height:80px;border-radius:4px;display:block}
.thumb-badge{position:absolute;top:2px;right:2px;background:rgba(0,0,0,.7);border-radius:50%;
  width:18px;height:18px;font-size:10px;display:flex;align-items:center;justify-content:center;color:#fff}
...
(function(){
  ...
  fetch('/settime?t='+encodeURIComponent(ts))
    .then(function(r){bar.textContent=r.ok?'Clock synced: '+ts:'Sync failed';
      bar.style.color=r.ok?'#00e5a0':'#cf3232';})
...
// queue holds {name, blob} — built one at a time
var queue=[];
var busy=false;

// yield: let browser paint/GC before next step
function yld(ms){ return new Promise(function(r){setTimeout(r,ms||30);}); }

// Process ONE file: load → draw to 240x320 canvas → toBlob → cleanup
  ...
  return new Promise(function(resolve, reject){
    ...
    img.onload=function(){
      ...
      try{
        ...
        var sc=Math.min(240/img.naturalWidth, 320/img.naturalHeight);
        var dw=Math.round(img.naturalWidth*sc);
        var dh=Math.round(img.naturalHeight*sc);
        ctx.fillStyle='#000';
        ctx.fillRect(0,0,240,320);
        ctx.drawImage(img,(240-dw)/2,(320-dh)/2,dw,dh);
        URL.revokeObjectURL(url);
        img.src=''; // release image memory before toBlob

        cv.toBlob(function(blob){
          if(!blob){ resolve(null); return; }
          var name=file.name.replace(/\.[^.]+$/,'')+'.jpg';
          // grab tiny 60x80 thumbnail pixel data for display, then discard big canvas
          var tc=document.createElement('canvas');
          tc.width=60; tc.height=80;
          tc.getContext('2d').drawImage(cv,0,0,60,80);
          cv.width=1; cv.height=1; // free big canvas memory
          resolve({name:name, blob:blob, thumb:tc});
        },'image/jpeg',0.85);
      } catch(e){
        URL.revokeObjectURL(url);
        reject(e);
    ...
    img.onerror=function(){
      ...
      clearTimeout(timer);
      URL.revokeObjectURL(url);
      reject(new Error('load error: '+file.name));
    };
...
// ── File picker: process strictly one at a time ───────────────
  ...
  for(var i=0;i<files.length;i++){
    ...
    qi.textContent=files[i].name;
    await yld(20); // let UI update before heavy work

    var result=null;
    try{ result=await processOne(files[i]); }
    catch(e){ console.warn(e); }

    if(result){
      queue.push({name:result.name, blob:result.blob});

      var wrap=document.createElement('div'); wrap.className='thumb-wrap';
      wrap.appendChild(result.thumb);
      var badge=document.createElement('div'); badge.className='thumb-badge';
      badge.id='b'+(queue.length-1);
      ...
      // show error slot
      var wrap=document.createElement('div'); wrap.className='thumb-wrap';
      wrap.style.border='1px solid #cf3232';
      var badge=document.createElement('div'); badge.className='thumb-badge';
      badge.textContent='!'; badge.style.background='#cf3232';
      wrap.appendChild(badge);
      thumbsEl.appendChild(wrap);
    }

    await yld(30); // yield after each image — lets GC run
  }

  var total=(queue.reduce(function(s,x){return s+x.blob.size;},0)/1024|0);
  st.textContent='Ready: '+queue.length+' / '+files.length+' processed, ~'+total+' KB';
  qi.textContent='';
  if(queue.length>0) sendBtn.disabled=false;
};
...
sendBtn.onclick=async function(){
  ...
  for(var i=0;i<queue.length;i++){
    var item=queue[i];
    qi.textContent='Uploading '+(i+1)+' of '+queue.length+': '+item.name;
    var badge=document.getElementById('b'+i);

    await new Promise(function(resolve){
      var fd=new FormData(); fd.append('file',item.blob,item.name);
      var xhr=new XMLHttpRequest();
      xhr.upload.onprogress=function(e){
        if(e.lengthComputable){
          var p=e.loaded*100/e.total|0;
          barEl.style.width=p+'%';
      ...
      xhr.onload=function(){
        ...
        st.textContent=(ok?'[OK] ':'[ERR] ')+item.name;
        if(badge) badge.textContent=ok?'✓':'!';
        if(badge) badge.style.background=ok?'#00c853':'#cf3232';
        resolve();
      };
    ...
    await yld(100); // small pause between uploads — lets ESP32 settle
  }

  prog.style.display='none';
  qi.textContent='';
  st.textContent='Done! '+queue.length+' image(s) uploaded.';
  busy=false; queue=[];
  fi.value='';
  loadList();
};
...
function loadList(){
  ...
  }).catch(function(){document.getElementById('files').innerHTML='<p style="color:#cf3232;padding:8px">Failed to load</p>';});
}
function del(name){
...
// Free the entire imageList and reset count
void freeImageList() {
    if (imageList) {
        for (int i = 0; i < imageCount; i++) free(imageList[i]);
        free(imageList);
        imageList = nullptr;
    }
    imageCount = 0;
...
// Append one path — reallocs the pointer array, then strdup the string
void addToList(const char* path) {
    // Deduplicate
    for (int i = 0; i < imageCount; i++)
        if (strcmp(imageList[i], path) == 0) return;

    char** tmp = (char**)realloc(imageList, (imageCount + 1) * sizeof(char*));
    if (!tmp) { Serial.println("addToList: realloc failed"); return; }
    imageList = tmp;

    imageList[imageCount] = strdup(path);
    if (!imageList[imageCount]) { Serial.println("addToList: strdup failed"); return; }
    imageCount++;
}

void scanSD() {
    freeImageList();   // release previous list before rebuilding
    File root = SD.open("/");
    if (!root) return;
    ...
    while (f) {
        if (!f.isDirectory() && isImage(f.name())) {
            char path[MAX_FNAME];
            snprintf(path, MAX_FNAME, "/%s", f.name());
            addToList(path);
        }
        f.close();
        f = root.openNextFile();
    ...
    root.close();
    Serial.printf("scanSD: found %d images\n", imageCount);
}
    ...
    for (;;) {
        if (sdOK && appMode == MODE_SLIDESHOW &&
            ...
            if (xSemaphoreTake(tftMutex, pdMS_TO_TICKS(500)) == pdTRUE) {
                ...
                if (ok) showOverlay(imageList[curImg] + 1);
                xSemaphoreGive(tftMutex);
            }
